...
fig1 = px.bar(
    ...
    title=f'Top {top_n} Negara Bagian dengan Pelanggan Terbanyak',
    color='jumlah',
...
with st.expander("💡 Lihat Insight"):
    ...
    - Lima negara bagian teratas (SP, RJ, MG, RS, PR) menyumbang sekitar **73%** dari total pelanggan.
...
fig2 = px.bar(
    ...
    title='Jumlah Pelanggan per Wilayah di Brasil',
    color='wilayah',
    text='jumlah'
)
...
with st.expander("💡 Lihat Insight"):
    ...
    - Wilayah **Norte** hanya memiliki sekitar **1.86%** pelanggan — penetrasi e-commerce masih sangat rendah.
    - Terdapat peluang ekspansi besar di wilayah Norte dan Nordeste.
    """)
...
with col2:
    top_kota_df = city_df.head(top_kota)

    fig4 = px.bar(
        top_kota_df,
        x='customer_city', y='jumlah_pelanggan',
        title=f'Top {top_kota} Kota dengan Pelanggan Terbanyak',
